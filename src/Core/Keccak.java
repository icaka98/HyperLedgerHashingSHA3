...
public class Keccak {
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    if ((5 * i + j) < (parameters.getR() / w)) {
                        Z.append(addZero(HexUtils.getReverseHex(S[j][i].toByteArray()), 16).substring(0, 16));
                    }
                }
            }
    ...
    private void doKeccackf(BigInteger[][] A) {
        for (int i = 0; i < n; i++) {
            A = roundB(A, RC[i]);
        }
    }
    ...
    private BigInteger[][] padding(String message, Parameters parameters) {
        ...
        StringBuilder messageBuilder = new StringBuilder(message + parameters.getD());
        while (((messageBuilder.length() / 2) * 8 % parameters.getR()) != ((parameters.getR() - 8))) {
            messageBuilder.append("00");
        }
